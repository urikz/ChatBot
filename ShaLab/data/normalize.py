...
from nltk.tokenize.treebank import MacIntyreContractions

WORDS_REPLACEMENT_RULES = {
    "n't": "not",
    "'ll": "will",
    "'s": "is",
    "'m": "am",
...
def normalize_after_tokenization(words, vocab):
    words2 = [
        WORDS_REPLACEMENT_RULES.get(w, w)
        for w in words
    ]
    return [
        spell(w) if w not in vocab else w
        for w in words2
    ]


class TreebankWordDetokenizer(object):
    """
    ...
    UPPERCASE_AFTER_PUNCTUATION = re.compile(r'([\.\?!])\s(\w+)')
    UPPERCASE_IN_THE_BEGINNING = re.compile(r'^(\w+)')
    UPPERCASE_I = [
        (re.compile(r' i '), (r' I ')),
        (re.compile(r'^i '), (r'I ')),
        (re.compile(r' i$'), (r' I')),
    ...
    def detokenize(self, tokens, convert_parentheses=False):
        ...
        text = self.UPPERCASE_AFTER_PUNCTUATION.sub(
            lambda x: x.group(1) + ' ' + x.group(2).title(),
            text,
        )
        text = self.UPPERCASE_IN_THE_BEGINNING.sub(
            lambda x: x.group(1).title(),
            text,
        )
        for regexp, substitution in self.UPPERCASE_I:
            text = regexp.sub(substitution, text)
